...
fn encoding_error(msg: String) -> TsldbError {
    TsldbError::EncodingError(msg.to_string()).into()
}
...
pub fn unmarshal_fixed_int<T: FixedInt>(src: &[u8]) -> Result<(T, &[u8])> {
    ...
    let v = T::decode_fixed(src);
    Ok((v, &src[T::REQUIRED_SPACE..]))
}
...
pub fn unmarshal_var_int<T: VarInt>(src: &[u8]) -> Result<(T, &[u8])> {
    match T::decode_var(src) {
        ...
        _ => Err(encoding_error("Error decoding var int".to_string())),
    }
}
...
pub fn unmarshal_bytes(src: &[u8]) -> Result<(&[u8], &[u8])> {
    match unmarshal_var_int::<usize>(src) {
        ...
        Err(err) => Err(encoding_error(format!("cannot unmarshal string size: {}", err))),
    }
}