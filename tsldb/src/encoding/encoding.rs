...
#[derive(Debug, Clone, PartialOrd, PartialEq, Copy, Serialize, Deserialize)]
...
impl TryFrom<u8> for MarshalType {
    ...
    fn try_from(v: u8) -> std::result::Result<Self, TsldbError> {
        match v {
            ...
            _ => Err(TsldbError::EncodingError(format!("Invalid marshal type value: {v}")))
        }
    }
}

/// check_precision_bits makes sure precision_bits is in the range [1..64].
pub(crate) fn check_precision_bits(precision_bits: u8) -> std::result::Result<(), TsldbError> {
    if !(1..=64).contains(&precision_bits) {
        return Err(TsldbError::CannotSerialize(format!(
...
pub fn unmarshal_timestamps(
    ...
    match unmarshal_int64_array(dst, src, mt, first_value, items_count) {
        ...
        Err(err) => {
            ...
            Err(TsldbError::CannotDeserialize(msg))
        }
    }
}
...
pub fn unmarshal_values(
    ...
    match unmarshal_int64_array(dst, src, mt, first_value, items_count) {
        Err(err) => Err(TsldbError::CannotDeserialize(format!(
...
fn cannot_unmarshal(msg: String) -> TsldbError {
    TsldbError::CannotDeserialize(msg)
}

fn decompression_error(msg: String) -> TsldbError {
    TsldbError::DecompressionFailed(msg)
}
...
pub fn marshal_int64_array(
    ...
    if a.is_empty() {
        return Err(TsldbError::CannotSerialize("BUG: a must contain at least one item".to_string()));
    }
        ...
        || (dst.len() - orig_len) > (0.9 * bb.len() as f64) as usize
    {
        ...
        mt = match mt {
            ...
            _ => return Err(TsldbError::CannotSerialize(format!("BUG: unexpected mt={}", mt))),
        };
...
pub fn unmarshal_int64_array(
    ...
    match mt {
        Lz4NearestDelta => match decompress_lz4(src) {
            Err(err) => Err(decompression_error(format!("cannot decompress lz4 data: {}", err))),
            Ok(uncompressed) => {
                match unmarshal_int64_nearest_delta(dst, &uncompressed, first_value, items_count) {
                    Err(err) => {
                        Err(cannot_unmarshal(format!(
        ...
        Lz4NearestDelta2 => match decompress_lz4(src) {
            Err(err) => Err(decompression_error(format!(
        ...
        NearestDelta => match unmarshal_int64_nearest_delta(dst, src, first_value, items_count) {
            Err(e) => Err(cannot_unmarshal(format!(
        ...
        Const => {
            if !src.is_empty() {
                return Err(cannot_unmarshal(format!(
...
pub fn unmarshal_bytes_fast(src: &[u8]) -> Result<(&[u8], &[u8])> {
    if src.is_empty() {
        return Err(cannot_unmarshal(
            "cannot decode size from src=; it must be at least 2 bytes".to_string(),
        ));
    }
    return match unmarshal_var_int::<usize>(src) {
        ...
        Err(err) => Err(cannot_unmarshal(format!(
    ...
    unmarshal_bytes_fast(src)
        .and_then(|(tail, bytes)| {
            let str = std::str::from_utf8(bytes)
                .map_err(|x| cannot_unmarshal(format!("cannot decode string from bytes: {}", x)))?;
